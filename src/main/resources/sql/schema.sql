...
CREATE TABLE IF NOT EXISTS article_words (
  article_id VARCHAR(256) REFERENCES articles(article_id) ON DELETE CASCADE,
  word_id BIGINT REFERENCES words(word_id) ON DELETE CASCADE,
  tf FLOAT
);

CREATE OR REPLACE FUNCTION idf(w_id BIGINT, total_articles_count BIGINT)
  RETURNS FLOAT AS $$
BEGIN
  RETURN (SELECT ln(total_articles_count / (SELECT articles_count FROM words WHERE word_id = w_id)::FLOAT));
END; $$
LANGUAGE 'plpgsql';
...
BEGIN
  ...
  SELECT SUM(a1.tf * (SELECT idf(a1.word_id, total_articles_count))
    * a2.tf * (SELECT idf(a2.word_id, total_articles_count))) INTO dividend
  FROM article_words a1 INNER JOIN article_words a2 ON a1.word_id = a2.word_id
  WHERE a1.article_id = a1_id AND a2.article_id = a2_id;
  SELECT SUM(a1.tf * (SELECT idf(a1.word_id, total_articles_count))
    * a1.tf * (SELECT idf(a1.word_id, total_articles_count))) INTO a1_length
  FROM article_words a1 WHERE a1.article_id = a1_id;
  SELECT SUM(a2.tf * (SELECT idf(a2.word_id, total_articles_count))
    * a2.tf * (SELECT idf(a2.word_id, total_articles_count))) INTO a2_length
